...
sys.path.append('../../')
import utils
from utils import prt_img_update, SupConLoss, prt_logits_update, kd_loss, gce
...
class BaseSolver:
	...
	def active_sampling(self, inds, candidate_dataset, aim_dataset):
		...
		active_samples = list()
		for i in inds:
			(data, _, _), target, path, _ = candidate_dataset[i]
	...
	def clue_query(self, num_active):
		...
		with torch.no_grad():
			for (x, _, _), y, _, idxs in self.tgt_loader:	
...
# 	def solve(self, epoch, curr_target_loader=None, curr_src_loader=None, idxs_lb=None):
# 		"""
# 		Unsupervised Domain adaptation via SENTRY: Selective Entropy Optimization via Committee Consistency
# 		"""
# 		# if True in idxs_lb:
# 		# 	self.tgt_loader = curr_target_loader
# 		# 	self.src_loader = curr_src_loader
# 		clr_loss = SupConLoss()
...
# 		self.tgt_loader.dataset
# 		self.net.train()
# 		queue = torch.zeros(self.queue_length).to(self.device)
# 		pointer = 0
# 		for batch_idx, (((data_s, _, _), label_s, _, _), ((_, data_t_og, data_t_raug), label_t, _, indices_t)) in enumerate(tqdm(joint_loader)):
...
# 			# xeloss_tgt_sup = lambda_src*nn.CrossEntropyLoss()(score_ts, label_ts)
# 			# loss += xeloss_tgt_sup

# 			# info_str += " Tgt_sup XE loss: {:.3f}".format(xeloss_tgt_sup.item()) 
...
# 			# img_feat_t = F.normalize(img_feat_t, dim=1)
# 			# const_loss = 0
# 			# for data_t_aug_curr in data_t_raug:
# 			# 	score_t_aug_curr, str_img_feat_t= self.net(data_t_aug_curr.to(self.device), with_emb=True)	
# 			# 	str_img_feat_t = F.normalize(str_img_feat_t, dim=1)
# 			# 	clr_feat = torch.cat((img_feat_t.unsqueeze(1), str_img_feat_t.unsqueeze(1)), 1)
# 			# 	const_loss += 0.1*clr_loss(clr_feat)
# 			# loss += const_loss
# 			# info_str += " const_loss: {:.3f}".format(const_loss.item())
# 			########################### KD loss ####################################
# 			# self.net.eval()
# 			# with torch.no_grad():
# 			# 	emd_feat_t, img_feat_t= self.net(data_t_og, with_emb=True)

# 			# 	probs_t = F.softmax(emd_feat_t, 1)

# 			# 	value, predicted = torch.max(probs_t, 1)
# 			# 	idx = torch.nonzero(value > 0.9)

# 			# if len(idx)>0:
# 			# 	req_img_t = data_t_og[idx]
# 			# 	req_img_t = req_img_t.squeeze(1).detach().clone().cuda()

# 			# 	list_prototype_logits = []
# 			# 	for i in range(len(idx)):
# 			# 		req_idx = idx[i].item()
# 			# 		list_prototype_logits.append(self.prototype_logits[predicted[req_idx]])
# 			# 	pseudo_prt_logits = torch.stack(list_prototype_logits, 0).detach().clone().cuda()

# 			# 	self.net.train()

# 			# 	emd_feat_t, img_feat_t = self.net(req_img_t, with_emb=True)

# 			# 	kd_loss_t = 0.1*kd_loss(emd_feat_t, pseudo_prt_logits)
# 			# 	loss +=  kd_loss_t
# 			# 	info_str += " kd_loss: {:.3f}".format(kd_loss_t.item())
# 			# self.net.train()
# 			############################ PL GCE loss ####################################
# 			# self.net.eval()
# 			# with torch.no_grad():
...
# 			# 	req_img_t = data_t_og[idx]
# 			# 	req_img_t = req_img_t.squeeze(1).detach().clone().cuda()
# 			# 	pseudo_gt = predicted[idx]
# 			# 	pseudo_gt = pseudo_gt.squeeze(1).detach().clone().cuda()
...
# 			# 	logits_t = F.softmax(emd_feat_t, 1)

# 			# 	pl_loss_t = 0.1*gce(logits_t, pseudo_gt)
# 			# 	loss +=  pl_loss_t
# 			# 	info_str += " pl_gce_loss: {:.3f}".format(pl_loss_t.item())
# 			# self.net.train()
#             ############################ Entropy Loss ####################################

#             # img_feat_t, ent_logits = self.clf(img_t)

#             # ent_loss = 0.1*Categorical(logits = ent_logits).entropy().mean(0)
#             # net_loss['ent_loss'] = ent_loss
#             # print_loss['ent_loss'] = ent_loss.item()
...
class SENTRYSolver(BaseSolver):
	...
	def solve(self, epoch):
		...
		queue = torch.zeros(self.queue_length).to(self.device)
		pointer = 0
		for batch_idx, (((data_s, _, data_s_raug), label_s, _, indices_s), ((_, data_t_og, data_t_raug), label_t, _, indices_t)) in enumerate(tqdm(joint_loader)):
